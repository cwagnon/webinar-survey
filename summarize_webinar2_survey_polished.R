# ============================================================
# Webinar 2 Survey Summary Script
# ------------------------------------------------------------
# Purpose:
...
# Optional manual ordering for plotted questions
manual_order <- list(
...
order_responses <- function(data, var_name) {
  if (var_name %in% names(manual_order)) {
    ...
    present <- unique(data$response)
    ordered_levels <- c(preferred[preferred %in% present], setdiff(sort(present), preferred))
    data |> mutate(response = factor(response, levels = ordered_levels))
  } else {
    data |> mutate(response = fct_infreq(response))
  }
}
...
if (length(open_ended_cols_present) > 0) {
  ...
  for (col in open_ended_cols_present) {
    ...
    out_file <- file.path(
      table_dir,
      paste0(make_safe_filename(col), "_responses.csv")
    )
...
# Also write one table per closed-ended question
...
for (col in plot_candidates) {
  ...
  if (col %in% names(manual_order)) {
    plot_dat <- tibble(response = manual_order[[col]]) |>
      left_join(plot_dat, by = "response") |>
  ...
  p <- ggplot(plot_dat, aes(x = response, y = n)) +
    ...
    labs(
      ...
      caption = "Percentages based on non-missing responses"
    ) +
    expand_limits(y = max(plot_dat$n) * 1.15 + 0.1) +
